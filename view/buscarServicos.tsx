  ...
  return (
    <div className="px-3 py-5 sm:p-6 lg:p-8 max-w-7xl mx-auto font-sans">
      ...
      <form onSubmit={executarPesquisa} className="flex flex-col sm:flex-row gap-3 mb-6">
        {/* A classe h-20 é aplicada no mobile, e sm:h-16 volta para o tamanho normal no desktop */}
        <div className="flex-1 flex items-center border border-gray-200 rounded-xl px-4 h-20 sm:h-16 bg-white shadow-sm focus-within:border-blue-500 transition min-w-0">
          <Search size={24} className="text-gray-400 shrink-0" /> {/* Ícone ligeiramente maior no mobile */}
          <input
            type="text"
            value={termo}
            onChange={e => setTermo(e.target.value)}
            placeholder="Buscar por nome, categoria ou cidade..."
            className="flex-1 min-w-0 ml-3 text-sm text-gray-700 outline-none placeholder:text-gray-400 placeholder:truncate"
          />
          {termo && (
            <button type="button" onClick={() => setTermo("")} className="text-gray-400 hover:text-gray-600 shrink-0">
              <X size={20} />
            </button>
          )}
        </div>

        {/* Ajuste o botão para acompanhar a mesma lógica de altura se desejar */}
        <button
          type="button"
          onClick={() => setMostrarFiltros(!mostrarFiltros)}
          className={`flex items-center justify-center gap-2 px-4 h-20 sm:h-16 rounded-xl border text-sm font-semibold transition cursor-pointer shrink-0 whitespace-nowrap ${mostrarFiltros || temFiltrosAtivos
      ...
      {mostrarFiltros && (
        <div className="bg-white border border-gray-100 rounded-2xl p-4 sm:p-6 mb-6 shadow-sm space-y-5">
          <div className="flex items-center justify-between gap-2">
            ...
            {temFiltrosAtivos && (
              <button onClick={limparFiltros} className="text-xs text-blue-600 font-bold hover:text-blue-700 cursor-pointer shrink-0">
                Limpar tudo
              </button>
            )}
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {["Todas", ...categorias.map((cat) => cat.nome_categoria)].map(cat => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition cursor-pointer ${categoriaSelecionada === cat
                      ? "bg-blue-600 text-white border-blue-600"
                      : "bg-white text-gray-600 border-gray-200 hover:border-blue-400"
                    }`}
                >
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`shrink-0 w-11 h-6 flex items-center rounded-full p-1 transition-colors duration-200 focus:outline-none ${apenasVerificados ? "bg-blue-600" : "bg-gray-200"
                }`}
            >
              <div className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${apenasVerificados ? "translate-x-5" : "translate-x-0"
                }`} />
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          {prestadoresFiltrados.map(prestador => (
            <Link
              ...
              className="bg-white border border-gray-100 rounded-2xl p-4 sm:p-5 shadow-sm hover:shadow-md transition group block min-w-0"
            >
              {/* Header do card */}
              <div className="flex items-center gap-3 sm:gap-4 mb-4 min-w-0">
                {prestador.foto_perfil ? (
                  ...
                  <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-blue-600 text-white flex items-center justify-center text-lg sm:text-xl font-bold shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {Boolean(prestador.status_verificado) && (
                      <span className="text-[9px] bg-green-50 text-green-600 font-bold px-1.5 py-0.5 rounded-full shrink-0 whitespace-nowrap">✓ Verificado</span>
                    )}
                  </div>
                  {prestador.categoria_principal && (
                    <span className="text-[11px] font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full mt-1 inline-block truncate max-w-full">
                      {prestador.categoria_principal}
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center justify-between gap-2 pt-3 border-t border-gray-50">
                {prestador.cidade ? (
                  <div className="flex items-center gap-1 text-[11px] text-gray-400 min-w-0">
                    ...
                    <span className="truncate">{prestador.cidade}</span>
                  </div>
                ) : <div />}
                <span className="text-xs font-bold text-blue-600 group-hover:text-blue-700 transition shrink-0 whitespace-nowrap">
                  Ver perfil →
                </span>
              </div>
            </Link>
